...
# Set lvm mount
CFD_DATA_FILE_SYSTEM='xfs'
CFD_DATA_MOUNT_OPTIONS='defaults,nofail,noatime,nodiratime,logbsize=256k,allocsize=64m'
cat >> /etc/fstab << EOF
...
CFD_DATA_MOUNT_OPTIONS='defaults,_netdev,noatime,nodiratime,hard,rsize=1048576,wsize=1048576,bg'
cat >> $CHROOT/etc/fstab << EOF
$sms_ip:$CFD_DATA_MOUNT_DIR  $CFD_DATA_MOUNT_DIR    $CFD_DATA_FILE_SYSTEM    $CFD_DATA_MOUNT_OPTIONS  0 0
EOF

CFD_DATA_EXPORT_OPTIONS='rw,no_root_squash,async,no_subtree_check'
cat >> /etc/exports << EOF
