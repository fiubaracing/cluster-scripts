...
if %NP% LSS 2 (
set COMMAND=transformPoints %TRANSFORM_ACTION% "%TRANSFORM_DATA%" %REGION% -noFunctionObjects -case "%CASE%"
) else (
set COMMAND=%MPICOMMAND% transformPoints -parallel %TRANSFORM_ACTION% "%TRANSFORM_DATA%" %REGION% -noFunctionObjects -case "%CASE%"
)
