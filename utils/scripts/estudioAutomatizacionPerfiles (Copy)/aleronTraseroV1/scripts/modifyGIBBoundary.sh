#!/bin/bash
echo "*****************************"
echo "*    Modify GIB Boundary    *"
echo "*****************************"

echo "CASE                  : $CASE"
echo "NP                    : $NP"
echo "LOG                   : $LOG"
echo "CONSTANT              : $CONSTANT"
echo "ALL_REGIONS           : $ALL_REGIONS"
echo "ENV_LOADER            : $ENV_LOADER"
echo "CORE_FOLDER           : $CORE_FOLDER"
echo "GUI_FOLDER            : $GUI_FOLDER"
echo "EXT_FOLDER            : $EXT_FOLDER"


. "$CASE/scripts/environment.conf"


echo "GIB_ARGUMENTS: $GIB_ARGUMENTS"
echo "GIB_VALUES: $GIB_VALUES"

if [[ "$NP" -lt "2" ]]; then
modifyGIBBoundary -constant $GIB_ARGUMENTS "$GIB_VALUES" -case "$CASE" 2>&1 | tee -a "$LOG"
else
$MPICOMMAND modifyGIBBoundary -constant $GIB_ARGUMENTS "$GIB_VALUES" -parallel -case "$CASE" 2>&1 | tee -a "$LOG"
fi


