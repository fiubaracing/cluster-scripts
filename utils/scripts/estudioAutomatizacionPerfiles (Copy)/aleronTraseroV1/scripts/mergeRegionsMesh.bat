...
if %NP% LSS 2 (
set COMMAND=mergeMeshes "%CASE%" "%CASE%" -overwrite -noFunctionObjects -allRegionsToMesh -case "%CASE%"
) else (
set COMMAND=%MPICOMMAND% mergeMeshes "%CASE%" "%CASE%" -parallel -overwrite -noFunctionObjects -allRegionsToMesh -case "%CASE%"
)
