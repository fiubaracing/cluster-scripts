...
if "%REGIONS%" == "" (
if %NP% LSS 2 (
set COMMAND=stretchMesh -noFunctionObjects -case "%CASE%"
) else (
set COMMAND=%MPICOMMAND% stretchMesh -noFunctionObjects -parallel -case "%CASE%"
)
) else (
if %NP% LSS 2 (
set COMMAND=stretchMesh %REGIONS% "%REGIONS_LIST%" -noFunctionObjects -case "%CASE%"
) else (
set COMMAND=%MPICOMMAND% stretchMesh %REGIONS% "%REGIONS_LIST%" -noFunctionObjects -parallel -case "%CASE%"
)
)
